...
def transform_and_draw_model(edges_Ro, intrinsic, extrinsic, fig_axis):
    ...
    u_1, v_1 = perspective_projection(intrinsic, matTools.transform_point_with_matrix(extrinsic, edges_Ro[:, [0, 1, 2]]))
    u_2, v_2 = perspective_projection(intrinsic, matTools.transform_point_with_matrix(extrinsic, edges_Ro[:, [3, 4, 5]]))
...
def perspective_projection(intrinsic, P_c):
    ...
    mat = np.dot(intrinsic, P_c.T)/P_c[:, 2]
    u, v = mat[0], mat[1]
...
def calculate_normal_vector(p1_Ri, p2_Ri, intrinsic):
    ...
    l1 = np.dot(np.linalg.inv(intrinsic), p1_Ri)
    l2 = np.dot(np.linalg.inv(intrinsic), p2_Ri) - l1
...
def calculate_error(nb_segments, normal_vectors, segments_Rc):
    ...
    for p in range(nb_segments):
        dot_prod = np.dot(normal_vectors[p, [0, 1, 2]], segments_Rc[p, [0, 1, 2]]) ** 2
        dot_prod2 = np.dot(normal_vectors[p, [0, 1, 2]], segments_Rc[p, [3, 4, 5]]) ** 2
        err = err + ( dot_prod + dot_prod2 )
