...
class SFM:
    ...
    def compute_pose_PNP(self, view):
        ...
        t = np.array([[0, 0, 0]]).T
        ################################################
        # 6.1 - compute new pose using solvePnPRansac
    ...
    def save_points(self):
        """Saves the reconstructed 3D points to xyz file"""

        number = len(self.done)
        ################################################
        # 5.2 - Store point cloud as .xyz file
        ## INSERT YOUR CODE HERE !!!!
    ...
    def reconstruct(self):
        ...
        self.save_points()
        logging.info("Points plotted for %d views : %d", len(self.done),  len(self.points_3D))

        #############################################################
        ## 6.2 - Uncomment the lines below
        ## Add the other views, one-by-one
        ...
        #     logging.info("Points plotted for %d views : %d", i+1,  len(self.points_3D))
        #
        # self.save_points()
